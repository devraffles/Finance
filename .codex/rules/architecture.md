...
- Route handlers em `frontend/src/app/api/**` devem ser adapters finos que importam funcoes server-side do pacote interno `@financas360/backend`.
- Apenas codigo server-side do frontend pode importar `@financas360/backend`; componentes visuais, hooks client e utilitarios de UI nao podem importar backend.
...
import { getSession } from "@financas360/backend/lib/auth";
import { criarConta } from "@financas360/backend/services/contas";
...
No exemplo acima, validacao Zod, ownership e acesso Prisma ficam em `@financas360/backend/services/contas`. O route handler apenas adapta HTTP para a funcao de dominio.
